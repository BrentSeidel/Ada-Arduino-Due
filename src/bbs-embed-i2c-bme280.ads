...
with BBS.embed;
use type BBS.embed.UInt12;
use type BBS.embed.int32;
use type BBS.embed.int64;
...
--
   ...
   procedure configure(self : not null access BME280_record'class;
                       i2c_port : i2c_interface; addr : addr7; error : out BBS.embed.i2c.err_code);
   --
   -- Starts the BME280 converting data.  Temperature, pressure, and humidity
   -- are converted at the same time.
   --
   procedure start_conversion(self : not null access BME280_record'class; error : out BBS.embed.i2c.err_code);
   --
   -- Check for data ready.  Reading a value before data is ready will have
   -- undesirable results.
   --
   function data_ready(self : not null access BME280_record'class; error : out BBS.embed.i2c.err_code) return boolean;
   --
   ...
   --
   procedure read_data(self : not null access BME280_record'class; error : out BBS.embed.i2c.err_code);
   --
